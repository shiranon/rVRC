...
import { clothPageAction } from './index'
...
describe('clothPageActionのテスト', () => {
	it('idに数字以外がある場合はエラーになる', async () => {
		const result = await clothPageAction({
	...
	it('intentがaddFolderの場合にaddClothを呼び出せる', async () => {
		const formData = new FormData()
		...
		expect(mockInitialize).toHaveBeenCalled()
		expect(mockAddCloth).toHaveBeenCalledWith(formData, '123')
	})
